...
export async function loginAction(
  ...
  try {
    ...
    if (!user || !user.passwordHash || user.deletedAt) {
      // Trois branches « comptes de demonstration » se trouvaient ici : si
      // l'e-mail saisi etait l'un de admin@fashlink.bj, contact@maisonadjovi.bj
      // ou awa.kone@example.bj, une session signee etait emise sans que le mot
      // de passe soit verifie. N'importe qui pouvait donc ouvrir une session
      // ADMIN en connaissant une adresse figurant dans le depot. Elles sont
      // retirees : pour parcourir les trois interfaces, on cree de vrais
      // comptes (voir `prisma/seed.ts`).
